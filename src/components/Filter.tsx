...
const Filter: FC<Filter> = ({ cars = [] }) => {
	...
	const filteredCars = cars
		.filter(({ brand, model, edition, year, engine, mileage, price, transmission, fuel, body, color }) => {
			...
			return brandFilter && modelFilter && editionFilter && minYearFilter && maxYearFilter && minEngineFilter && maxEngineFilter && minMileageFilter && maxMileageFilter && minPriceFilter && maxPriceFilter && transmissionFilter && fuelFilter && bodyFilter && colorFilter && optionFilter;
		})
		...
		if (filterData.brandIds) {
			...
			const minPricesData = filteredCars
				.map(({ price }, index) => {
					return {
						id: index,
						minPrice: String(price / 10000 * 10000)
					}
				})
				.filter((value, index, self) =>
			...
			const maxPricesData = filteredCars
				.map(({ price }, index) => {
					return {
						id: index,
						maxPrice: String(price / 10000 * 10000)
					}
				})
				.filter((value, index, self) =>
